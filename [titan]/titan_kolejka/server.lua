...
function GetMessage(steamID)
	local msg = ""
	local getplate = GetPlace(steamID)
	local procent = 100 / #waiting

	if GetPoints(steamID) ~= nil then
		msg = msg .. Config.Position .. "".. #waiting .. " graczy twoja szansa na wylosowanie wynosi : " .. procent .. "% (" .. Config.Kropki .. ")"
		if getplate < 2 then
		 msg = "Jesteś następny w kolejce (połącz się ponownie jeżeli kropki przestaną postępować)"
		end
