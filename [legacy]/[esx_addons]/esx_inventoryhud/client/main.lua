...
RegisterNUICallback("GetNearPlayersPolice", function(data, cb)
    ...
    if not foundPlayers then
        exports.pNotify:SendNotification(
            {
                text = '<strong class="red-text">Nie ma graczy w pobliżu</strong>',
                type = "error",
                timeout = 3000,
                layout = "bottomCenter",
                queue = "inventoryhud"
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        if not foundPlayers then
            exports.pNotify:SendNotification(
                {
                    text = '<strong class="red-text">Nie znaleziono gracza</strong>',
                    type = "error",
                    timeout = 3000,
                    layout = "bottomCenter",
                    queue = "inventoryhud"
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        if foundPlayer then
            ...
            exports.pNotify:SendNotification(
                {
                    text = '<strong class="red-text">Brak graczy w pobliżu</strong>',
                    type = "error",
                    timeout = 3000,
                    layout = "bottomCenter",
                    queue = "inventoryhud"
